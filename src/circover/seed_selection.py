...
Composite score = BC + AGTP - w_jsd * JSD_bar - w_z * Z

BC = Bhattacharyya Coefficient (Bhattacharyya, 1943)
"""
...
from .bhattacharyya import BhattacharyyaCoefficient
...
class GeometricSeedSelector:
    ...
    n_bins : int, default=30
        Histogram bins for BC, JSD, T_sim.
    ...
    def select(self, X_min: np.ndarray) -> tuple[np.ndarray, float]:
        ...
        # --- Candidate search ---
        bc_scorer = BhattacharyyaCoefficient(n_bins=self.n_bins)
        best_score = -np.inf
        best_idx = None
        ...
        for _ in range(self.n_candidates):
            ...
            Xs = X_score[idx]
            score = self._composite_score(X_score, Xs, bc_scorer)
            if score > best_score:
                best_score = score
                best_idx = idx
    ...
    def _composite_score(
        self, X: np.ndarray, Xs: np.ndarray, bc: BhattacharyyaCoefficient
    ) -> float:
        """Score = BC + AGTP - w_jsd*JSD - w_z*Z  (Eq. 6.4)"""
        bc_val = bc.score(X, Xs)
        agtp_val = self._agtp(X, Xs)
        jsd_val = self._jsd_mean(X, Xs)
        z_val = self._smoothness_z(Xs)
        return bc_val + agtp_val - self.w_jsd * jsd_val - self.w_z * z_val
